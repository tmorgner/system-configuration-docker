...
pihole_cmd="docker exec pihole pihole"
pihole_address="${pihole.ip}"
pihole_config="/etc/pihole/pihole"
...
GetFTLData() {
    ...
    if [[ -n "$pihole_address" ]]; then
        # Open connection to FTL
        exec 3<>"/dev/tcp/$pihole_address"
...
GetPiholeInformation() {
  # Get Pi-hole status
  pihole_web_status=$($pihole_cmd status web)
  ...
  if [[ ${pihole_web_status} == "1" ]]; then
    ...
    echo "Unkown pihole status? - '$pihole_web_status'"
...
GetVersionInformation() {
  ...
  if [ -e "/var/tmp/piHoleVersion" ]; then # the file exists...
    # the file exits, use it
    # shellcheck disable=SC1091
    source /var/tmp/piHoleVersion
    ...
    read -r -a core_versions <<< "$($pihole_cmd -v -p)"
    ...
    read -r -a web_versions <<< "$($pihole_cmd -v -a)"
    ...
    read -r -a ftl_versions <<< "$($pihole_cmd -v -f)"
    ...
    echo "last_check=${today}" > /var/tmp/piHoleVersion
    {
...
if [[ $# = 0 ]]; then
  ...
  # shellcheck disable=SC1091
  . /etc/pihole/pihole/setupVars.conf
